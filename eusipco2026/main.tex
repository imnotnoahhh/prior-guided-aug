...
\usepackage[
    pdfauthor={Fuyao Qin},
    pdftitle={Rethinking Data Augmentation for Small-Sample Learning},
    pdfsubject={},
    pdfkeywords={},
...
\sloppy  % Suppress underfull hbox warnings
...
\begin{itemize}
    ...
    \item \textbf{Validation:} Through Stratified 5-fold cross-validation, we show that single-operation policies offer the best reliability in data-scarce regimes (Section~IV).
\end{itemize}
...
\textbf{Understanding Augmentation Effects.} Recent work reveals \textit{why} augmentation works. Lin et al.~\cite{lin2024spectral} show augmentation operates through implicit spectral regularization. Yang et al.~\cite{yang2023sample} demonstrate that augmentation consistency is more sample-efficient. Our findings align: simpler augmentations provide stable regularization when data is limited.
...
\textbf{Best Policy Found.} The SAS protocol identifies a single operation as optimal: \textbf{ColorJitter} with Magnitude $m=0.2575$ and application Probability $p=0.4239$. Following standard practice, we tie the brightness, contrast, and saturation parameters to $m$, yielding $(b,c,s,h)=(m,m,m,0)$ where $h$ (hue) is disabled.

\textbf{Baseline Definition.} Baseline (S0) in Table~\ref{tab:main_results} includes standard augmentations: RandomHorizontalFlip and RandomCrop ($32 \times 32$ with 4-pixel padding), which are applied to all methods. Baseline-NoAug uses only center crop and normalization.
...
\begin{table}[htbp]
    \centering
    \caption{Comparison on CIFAR-100 ($\sim$90 samples/class). Results are mean $\pm$ std over 5 disjoint folds; std measures \textit{split sensitivity} (variation across different data partitions). \textbf{CV} = Std/Mean. \textbf{Width} = max $-$ min across folds.}
    ...
    \resizebox{\linewidth}{!}{
        \begin{tabular}{lccccc}
            ...
            Cutout & 36.26 & 1.23 & 3.0 & 3.39 & Med \\
            \midrule
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccccc}
        ...
        \midrule
        Phase A only (Sobol) & 35.80 & 1.65 & 4.8 & 4.61 & 1h \\
...
Our stability-aware search protocol (SAS) explicitly addresses this challenge by penalizing variance during policy selection. The resulting policy---a single, carefully-tuned ColorJitter operation---achieves competitive accuracy (96\% of RandAugment) with 33\% lower standard deviation and significantly better coefficient of variation (CV=1.91\% vs.\ 2.77\%). Analysis of semantic preservation metrics (SSIM, LPIPS) reveals a correlation between lower structural preservation and higher variance, suggesting that SAS provides a more robust estimation of true generalization.

These findings have practical implications for applications where predictability matters as much as accuracy. When practitioners value reproducibility and cannot afford multiple training runs, simpler augmentation policies offer a compelling alternative in small-sample regimes.
