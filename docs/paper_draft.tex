...
\begin{abstract}
We challenge the assumption that complexity equals performance in data augmentation. Standard automated strategies like RandAugment rely on complex, multi-operation policies that often fail when data is scarce. By restricting our focus to a CIFAR-100 regime with only 100 samples per class, we uncover a direct trade-off: complex augmentations increase training variance, nullifying their potential gains. Instead of searching for complex combinations, our three-stage stability-aware protocol identifies that a single, aggressively tuned operation (ColorJitter) matches RandAugment's accuracy (40.74\% vs 42.24\%) while cutting variance by 33\%. These results suggest that in small-sample learning, stability is the primary objective, not complexity.
\end{abstract}

\section{Introduction}

When training data is limited to fewer than 100 samples per class, standard assumptions about data augmentation break down. In widely used benchmarks like ImageNet, the prevailing logic is that ``more is better''---more operations and higher magnitudes lead to better generalization. However, our empirical analysis on low-shot CIFAR-100 reveals that these complex strategies introduce a dangerous amount of instability. Heavy augmentations like RandAugment can destroy the semantic integrity of the few available samples, leading to models that oscillate wildly between folds.

We investigate this phenomenon by asking: are these complex strategies necessary---or even harmful---when data is extremely limited? 
...
In this paper, we conduct a systematic study on CIFAR-100 with only 100 samples per class. We observe that while RandAugment provides a marginal accuracy gain, it introduces significant instability. Our search protocol explicitly targets stability to counter this issue. By filtering out variance-inducing operations early, we demonstrate that a minimalist approach often yields the most robust models.

Our search did not simply try to maximize accuracy. It explored a continuous Magnitude-Probability space, employed multi-fidelity optimization (ASHA) to discard bad actors quickly, and used a greedy selection process that explicitly penalized variance. Surprisingly, this rigorous process consistently selected a single operation, rejecting complex combinations. This suggests that in small-sample regimes, the simplest effective augmentation may be the most robust.
...
\textbf{Small-Sample Learning.} In scenarios where data is scarce, models often suffer from high variance. While transfer learning is a common fix, it doesn't apply when domains don't match, forcing us to train from scratch. In this regime, heavy augmentations (like Cutout \cite{devries2017improved}) can destroy the limited semantic information available. Our work follows the principles of ``Data-Centric AI,'' seeking to adjust the data distribution to better match model capacity. Our work also relates to recent discussions on simplicity and negative results in model design.
...
To navigate the stability-accuracy trade-off, we did not perform a standard grid search. Instead, we designed a three-stage funnel. 

\textbf{Phase A: Screening.} We began with a low-fidelity scan using Sobol sequences. This allowed us to immediately drop unstable operations (e.g., high-magnitude geometric distortions) before wasting significant compute.

\textbf{Phase B: Tuning.} For the candidates that pass this screen, we apply ASHA \cite{li2020system} to fine-tune them. This step dynamically focuses our computing budget on the best performers, killing off weak configurations early. This precise tuning of $(m, p)$ was achieved at a fraction of the cost of a full grid search.

\textbf{Phase C: Composition.} Finally, we look at composition (Algorithm \ref{alg:phase_c}). We use a tailored greedy strategy where a new operation is rejected unless its accuracy boost is significantly larger than the variance cost. Interestingly, this check almost always rejected multi-operation policies, converging repeatedly on single, stable transformations.
...
\subsection{Setup}
\textbf{Dataset Splits.} For our dataset, we took CIFAR-100 and limited it to 100 images per class, giving us 10,000 images in total. To prevent data leakage, we set up a strict validation protocol. We divided the data into 5 folds, keeping a 90/10 split for training and validation within each fold, and we report the final performance as the mean of these independent runs.

\textbf{Training.} We use \textbf{ResNet-18} trained from scratch for 200 epochs (SGD, momentum 0.9, weight decay set to $0.01$, and a batch size of 128). All experiments were performed on a single \textbf{NVIDIA A10 GPU}.
...
Why do complex strategies fail here? Two reasons stand out. First, \textbf{Validation Overfitting}: with scarce data, the search algorithm easily exploits noise in the small validation set, selecting parameters that fail to generalize. Second, \textbf{Loss of Implicit Priors}: the default RandAugment parameters ($N=2, M=9$) effectively serve as a strong prior derived from large-scale datasets (ImageNet). Blindly searching from scratch discards this inductive bias. Our single-op policy works because it stays within a safe ``semantic corridor,'' modifying images enough to teach invariance without confusing the model with meaningless noise.
