...
\textbf{Phase A: Screening.} We began with a low-fidelity screening, using Sobol sequences to quasi-randomly sample the parameter space. This quickly flagged and discarded operations that caused immediate instability (e.g., high-magnitude geometric distortions).

\textbf{Phase B: Tuning.} Survivors were then passed to the Asynchronous Successive Halving Algorithm (ASHA) \cite{li2020system}. ASHA allowed us to dynamically shift our computing budget, iterating rapidly on promising candidates while killing off underperformers early. This fine-tuning of $(m, p)$ was achieved at a fraction of the cost of a full grid search.

\textbf{Phase C: Composition.} Finally, we employed a greedy composition strategy (Algorithm \ref{alg:phase_c}). We attempted to stack operations, but with a strict rule: a new operation could only be added if its accuracy gain outweighed the additional variance it introduced. Interestingly, this check almost always rejected multi-operation policies, converging repeatedly on single, stable transformations.
